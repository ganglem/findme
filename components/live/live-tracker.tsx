...
export function LiveTracker({ stages, locations: initialLocations, userId }: LiveTrackerProps) {
  ...
  // Finde den aktuellen Standort des Benutzers
            ...
            <Card key={stage.id}>
              <CardHeader className="pb-2">
                <CardTitle className="text-lg flex justify-between">
                  ...
                  >
                    <MapPin className="w-4 h-4 mr-2" />
                    {userLocation?.stage_id === stage.id ? "Ich bin hier" : "Hier bin ich"}
                  </Button>
                </CardTitle>
              </CardHeader>
              <CardContent>
                      ...
                      {usersAtStage.map((location) => (
                        <Avatar key={location.user_id}>
                          <AvatarImage
                            ...
                            alt={location.profiles.username || "User"}
                          />
                          <AvatarFallback>{location.profiles.username?.[0]?.toUpperCase() || "U"}</AvatarFallback>
                        </Avatar>
                      ))}
                    </div>
                  ) : (
